function [egp_vec, time_tot_egp] = EGP_res(dataset, MC_runs)
    ...
    if dataset == "network delay"
        load(fullfile('Datasets/Network Delay','NetDelayfeat.mat'))
        for i=1:MC_runs
		   load(fullfile('Params. datasets','params_delay.m'))
           ...
           load(fullfile('Params. datasets','params_delay_ego.m'))
           tic
           ...
           load(fullfile('Params. datasets','params_delay_feat.m'))
           Xf = adj_feat(X,F);
     ...
     if dataset == "temperature"
	    load(fullfile('Datasets/Temperature','Tempfeatdata.mat'))
        for i=1:MC_runs
		   load(fullfile('Params. datasets','params_temp.m'))
           [se,nmse,time_egp,y_comb,w_vec,w_log,var_comb,loglike,time_log,test_rep]=main_EGP_run(X,y,params);
           time_tot_egp = time_tot_egp + time_egp;
           egp_mat = [egp_mat nmse'];
           Xf = adj_feat(X,F);
     ...
     if dataset == "email eu"
	    load(fullfile('Datasets/Temperature','emailEuData.mat'))
        for i=1:MC_runs
		   load(fullfile('Params. datasets','params_emaileu.m'))
           [se,nmse,time_egp,y_comb,w_vec,w_log,var_comb,loglike,time_log,test_rep]=main_EGP_run(X,y,params);
           time_tot_egp = time_tot_egp + time_egp;
           egp_mat = [egp_mat nmse'];
